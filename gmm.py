...
class GaussianMixture(torch.nn.Module):
    ...
    def predict(self, x, probs=False):
        ...
        weighted_log_prob = self._estimate_log_prob(x) + torch.log(self.pi.cuda())
        # don’use self.pi
        #weighted_log_prob = self._estimate_log_prob(x)

        if probs:
            #log_prob_max = torch.max(weighted_log_prob, dim=1, keepdim=True)[0]
            #p_k = torch.exp(weighted_log_prob - log_prob_max)
            p_k = weighted_log_prob
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "diag":
            ...
            mu = self.mu.cuda()
            x = x.cuda()
            prec = torch.rsqrt(var+self.eps).cuda()
